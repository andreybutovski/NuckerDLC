...
@FunctionRegister(name = "TridentPredictions", type = Category.Render)
public class TridentPredictions extends Function {

    public TridentPredictions() {
        addSettings();
    }

    record TridentPoint(Vector3d position, int ticks) {}

    final List<TridentPoint> tridentPoints = new ArrayList<>();

    @Subscribe
    public void renderPrediction(EventDisplay e) {
        for (TridentPoint tridentPoint : tridentPoints) {
            Vector3d pos = tridentPoint.position;
            Vector2f projection = ProjectionUtil.project(pos.x, pos.y - 0.3F, pos.z);
            int ticks = tridentPoint.ticks;
            ...
            DisplayUtils.drawImage(new ResourceLocation("textures/item/trident.png"), (int) posX + 5 , (int) posY - 2, 11 , 11, -1);
        ...
        for (Entity entity : mc.world.getAllEntities()) {
            if (entity instanceof TridentEntity trident) {
                ...
                for (int i = 0; i < 150; i++) {
                    prevPos = pos;
                    pos = pos.add(motion);
                    motion = getNextMotion(trident, motion);
                    ...
                    if (blockHitResult.getType() == BlockRayTraceResult.Type.BLOCK || pos.y < -128) {
                        tridentPoints.add(new TridentPoint(pos, ticks));
                        break;
                    }
                    ticks++;
                }
    ...
    private Vector3d getNextMotion(TridentEntity trident, Vector3d motion) {
