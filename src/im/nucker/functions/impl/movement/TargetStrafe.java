...
import im.nucker.utils.player.DamagePlayerUtil;
import im.nucker.utils.player.MoveUtils;
import im.nucker.utils.player.StrafeMovement;
...
public class TargetStrafe extends Function {
    ...
    public void onMotion(final MovingEvent event) {
        ...
        if (target != null && target.isAlive() && target.getHealth() > 0.0f) {
            ...
            final double speed = Math.min(strafeMovement.calculateSpeed(event, damageBoostSetting.get(),
                    damageUtil.isNormalDamage(), true, damageSpeed), 0.2);
    ...
    private LivingEntity getTarget() {
        return killAura.isState() ? (LivingEntity) killAura.getTarget() : null;
    }
