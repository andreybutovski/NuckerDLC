...
import im.nucker.command.impl.CommandException;
import im.nucker.MacroManager;
...
public class MacroCommand implements Command, MultiNamedCommand, CommandWithAdvice {
    ...
    private void removeMacro(Parameters parameters) {
        ...
        NuckerDLC.getInstance().getMacroManager().deleteMacro(macroName);
    ...
    private void clearMacros() {
        NuckerDLC.getInstance().getMacroManager().clearList();
        logger.log(TextFormatting.GREEN + "Все макросы были удалены.");
    }

    private void printMacrosList() {
        if (NuckerDLC.getInstance().getMacroManager().isEmpty()) {
            logger.log(TextFormatting.RED + "Список пустой");
            return;
        }
        NuckerDLC.getInstance().getMacroManager().macroList
