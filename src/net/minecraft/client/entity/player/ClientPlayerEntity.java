...
import im.nucker.command.CommandDispatcher;
import im.nucker.command.impl.DispatchResult;
import im.nucker.events.*;
import im.nucker.functions.api.FunctionRegistry;
import im.nucker.functions.impl.misc.NoPush;
import im.nucker.functions.impl.misc.Unhook;
import im.nucker.functions.impl.movement.AutoSprint;
...
public class ClientPlayerEntity extends AbstractClientPlayerEntity {
    ...
    public void tick() {
        if (this.world.isBlockLoaded(new BlockPos(this.getPosX(), 0.0D, this.getPosZ()))) {
            if (!NuckerDLC.getInstance().playerOnServer) {
                NuckerDLC.getInstance().playerOnServer = true;
            }

            NuckerDLC.getInstance().getEventBus().post(eventUpdate);
            super.tick();
    ...
    private void onUpdateWalkingPlayer() {
        ...
        NuckerDLC.getInstance().getEventBus().post(ea);
        ...
        if (this.isCurrentViewEntity()) {
            ...
            NuckerDLC.getInstance().getEventBus().post(eventMotion);
        ...
        NuckerDLC.getInstance().getEventBus().post(new EventPostMotion());
    ...
    public void sendChatMessage(String message) {
        ...
        CommandDispatcher commandDispatcher = NuckerDLC.getInstance().getCommandDispatcher();

        Unhook selfDestruct = functionRegistry.getSelfDestruct();
    ...
    public void closeScreen() {
        ...
        NuckerDLC.getInstance().getEventBus().post(event);
        if (!event.isCancel()) {
    ...
    private boolean shouldBlockPushPlayer(BlockPos pos) {
        FunctionRegistry functionRegistry = NuckerDLC.getInstance().getFunctionRegistry();
        NoPush antiPush = functionRegistry.getAntiPush();
    ...
    public void setSprinting(boolean sprinting) {
        NuckerDLC.getInstance().getEventBus().post(sprintEvent);
    ...
    public void livingTick() {
        ...
        if (this.isHandActive() && !this.isPassenger()) {
            NuckerDLC.getInstance().getEventBus().post(noSlowEvent);
