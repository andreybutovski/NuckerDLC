...
import im.nucker.events.EventCalculateCooldown;
import im.nucker.events.EventCooldown;
...
public class CooldownTracker {
    ...
    public float getCooldown(Item itemIn, float partialTicks) {
        ...
        NuckerDLC.getInstance().getEventBus().post(cooldown);
        ...
        if (!this.cooldowns.isEmpty()) {
            ...
            while (iterator.hasNext()) {
                ...
                if ((entry.getValue()).expireTicks <= this.ticks) {
                    ...
                    NuckerDLC.getInstance().getEventBus().post(eventCooldown);
                    iterator.remove();
